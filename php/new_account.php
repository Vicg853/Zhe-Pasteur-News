...
if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true){
    ...
    <div class="dropdown-menu dropdown-menu-right dropdown-secondary" aria-labelledby="navbarDropdownMenuLink-55">
      ...
      <a  class="nav-link text-info" href="#" data-toggle="modal" data-target="#modalLoginOrAccountForm" >Mon-compte</a>
    ...
    $LogedMyAccountResult = '<div class="modal-dialog cascading-modal" role="document">
    <div class="modal-content">
     ...
     ';
    $data=array("LoginAvatarResult"=>$LogedAvatarDivResult, "Login_Account_CreateFormsresult"=>$LogedMyAccountResult);
    ...
    $password_verification = $_POST['new_password_verification'];
    ...
    $mainLoginFormResult1 = '<div class="modal-dialog cascading-modal" role="document">
    <div class="modal-content">
      <div class="modal-c-tabs">
        <ul class="nav nav-tabs md-tabs tabs-2" role="tablist">
          <li class="nav-item">
            <a class="nav-link blue-gradient text-white" data-toggle="tab" href="#panel7" role="tab">
              Faire Login</a>
          </li>
          <li class="nav-item">
            <a class="nav-link blue-gradient text-white active" data-toggle="tab" href="#panel8" role="tab">
              Créer un compte</a>
          </li>
        </ul>
        <div class="tab-content">
                  ...
                  $.ajax({
                    ...
                    success: function( results ){
                      ...
                      $("#modalLRForm").append(obj.LoginFormresult);
                                 ...
                                 <!--Create account pannel-->
                                 <div class="tab-pane fade active" id="panel8" role="tabpanel">
                                   <div class="modal-body">
                                     <p class="h4 mb-4 text-danger">';
                                   
                                             $mainLoginFormResult2 = '
                   </p>
    ...
    if(empty($firstName) && empty($lastName) && empty($email) && empty($login) && empty($password) && empty($password_verification)){
      $data=array("LoginAvatarResult"=>$mainAvatarDivResult, "Login_Account_CreateFormsresult"=>$mainLoginFormResult1."Vous devez remplir toutes les options !".$mainLoginFormResult2);
      echo json_encode($data);
      die();
    }else{
        if($password == $password_verification){
            ...
            from `usersandadmins` where 
            `Login` = '".$login."';");
            if($query->execute() && $row = $query->fetch()){
                $data=array("LoginAvatarResult"=>$mainAvatarDivResult, "Login_Account_CreateFormsresult"=>$mainLoginFormResult1."Ce login est dejá en utilisation !".$mainLoginFormResult2);
                echo json_encode($data);
                die();
            }else{
                ...
                from `usersandadmins` where 
                `Email` = '".$email."';");
                if($query1->execute() && $row1 = $query->fetch()){
                    $data=array("LoginAvatarResult"=>$mainAvatarDivResult, "Login_Account_CreateFormsresult"=>$mainLoginFormResult1."Cet adresse mail est dejá en utilisation !".$mainLoginFormResult2);
                    echo json_encode($data);
                    die();
                }else{
                    $query2 = $conn->prepare("INSERT INTO `usersandadmins` VALUES (NULL, '".$firstName."', '".$lastName."', '".$login."', '".sha1($password)."', '".$email."', '0', '0', '0', '0', '".date("j, n, Y")."');");
                    if($query2->execute()){
                        $query3 = $conn->prepare("UPDATE `usersandadmins` SET `Status` = '1' WHERE `Login` = '".$login."';");
                        $query3->execute();
                        $_SESSION['userFirstName'] = $firstName;
                        $_SESSION['userLastName'] = $lastName;
                        $_SESSION['userImgLink'] = 0;
                        $_SESSION['userAdmissionDate'] = date("j, n, Y");
    ...
    $LogedMyAccountResult = '<div class="modal-dialog cascading-modal" role="document">
                      ...
                      die();
                        ...
                        $data=array("LoginAvatarResult"=>$mainAvatarDivResult, "Login_Account_CreateFormsresult"=>$mainLoginFormResult1."Oups, erreur l'ors de lenregistrement, réssaye, si l'erreur persite, contacté nous !".$mainLoginFormResult2);
                        echo json_encode($data);
                        die();
                    }   
                }
          ...
          $data=array("LoginAvatarResult"=>$mainAvatarDivResult, "Login_Account_CreateFormsresult"=>$mainLoginFormResult1."Les mot-de-passe ne correspondent pas !".$mainLoginFormResult2);
          echo json_encode($data);
          die();
        }
    }
