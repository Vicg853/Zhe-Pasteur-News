...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true){
    ...
    <div class="modal-dialog cascading-modal" role="document">
        <div class="modal-content">
         ...
         ';
    $data=array("LoginAvatarResult"=>$logedLoginAvatarOutput, "LoginFormresult"=>$logedLoginFormOutput);
    echo json_encode($data);
    die();
}else{
    $loginUser = $_POST['user_name'];
    $Password = $_POST['password'];
    //Output to AJAX if login or password are wrong incomplete etc...
    ...
    if ($loginUser == ""){
        $data=array("LoginAvatarResult"=>$mainLoginAvatarOutput, "LoginFormresult"=>$mainLoginFormOutputpart1."Vous devez informer votre login et/ou mot-de-passe !".$mainLoginFormOutputpart2);
        echo json_encode($data);
        session_destroy();
        die();
    }else{
        if($Password == ""){
            ...
            $conn = new PDO();
            $query = $conn->prepare("select * from `usersandadmins` where `Login` = '".$loginUser."';");
            if($query->execute() && $row = $query->fetch()){
                if($row['Password'] == sha1($Password)){
                    $_SESSION['userFirstName'] = $row['FirstName'];
                    $_SESSION['userLastName'] = $row['LastName'];
                    $_SESSION['userImgLink'] = $row['ProfileImgLink'];
                    $_SESSION['userAdmissionDate'] = $row['Admission'];
                    $_SESSION['userMail'] = $row['Email'];
                    $_SESSION['userLogin'] = $loginUser;
                    $_SESSION['loggedin'] = true;
                    $Query1 = $conn->prepare("UPDATE `usersandadmins` SET `Status` = '1' WHERE `Login` = '".$loginUser."';");
                    $Query1->execute();
                    //Output to AJAX if user has logged in
                    $logedLoginAvatarOutput = '
                    <a class="nav-link dropdown-toggle" id="navbarDropdownMenuLink-55" data-toggle="dropdown" aria-haspopup="true"
                      aria-expanded="false">
                      <img src="users/img/'.$_SESSION['userImgLink'].'" class="rounded-circle img-fluid" alt="avatar image" style="width: 50px; height: 50px; margin-left: 20px;">
                    ...
                    $logedLoginFormOutput = '<div class="modal-dialog" role="document">
                    <div class="modal-dialog cascading-modal" role="document">
                      <div class="modal-content">
                        <div class="modal-c-tabs">
                          <ul class="nav nav-tabs md-tabs tabs-2" role="tablist">
                 ...
                 ';
                    ...
                    $data=array("LoginAvatarResult"=>$mainLoginAvatarOutput, "LoginFormresult"=>$mainLoginFormOutputpart1."Login et/ou mot-de-passe incorrects !".$mainLoginFormOutputpart2);
                    echo json_encode($data);
                    session_destroy();
                    die();
                }
